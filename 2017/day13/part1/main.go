...
// a third layer which begins at depth 4 (with range 4), and a fourth
...
// scanner takes one picosecond to move one step. Drawing scanners as S,
...
//	[S]     [S]
//
// Your plan is to hitch a ride on a packet about to move through
// the firewall. The packet will travel along the top of each layer,
// and it moves at one layer per picosecond. Each picosecond, the
// packet moves one layer forward (its first move takes it into layer 0),
// and then the scanners move one step. If there is a scanner at the top
// of the layer as your packet enters it, you are caught. (If a scanner
// moves into the top of its layer while you are there, you are not caught:
// it doesn't have time to notice you before you leave.) If you were to do
...
func main() {
	...
	inputMap := parseInput(fileScanner)
	fmt.Printf("Input: %v\n", inputMap)
}
...
func parseInput(fileScanner *bufio.Scanner) map[int]int {
	inputMap := make(map[int]int)
	for fileScanner.Scan() {
		currLine := fileScanner.Text()
		depth, r := parseLine(currLine)
		inputMap[depth] = r
	}
	return inputMap
}
...
func parseLine(line string) (int, int) {
	...
	for i := 0; i < n; i++ {
		...
		for j := i; j < n; j++ {
			if string(line[j]) == " " {
				num := convStrToInt(curr[0 : len(curr)-1])
				arr = append(arr, num)
				i = j
				break
			}
