...
func calculateOxygenRatings(input []string) string {
	oxygens := input
	i := 0
	for len(oxygens) > 1 {
		m, n := len(oxygens), len(oxygens[0])
		if i >= n {
			break
		}
		freqmap := make(map[string]int)
		for j := 0; j < m; j++ {
			freqmap[string(oxygens[j][i])]++
		}
		newOxygens := []string{}
		if freqmap["1"] == freqmap["0"] { // keep the oxygens with 1
			for k := 0; k < m; k++ {
				if string(oxygens[k][i]) == "0" {
					continue
				}
				newOxygens = append(newOxygens, oxygens[k])
			}
		}
...
func calculateCarbonRatings(input []string) string {
	carbons := input
	i := 0
	for len(carbons) > 1 {
		m, n := len(carbons), len(carbons[0])
		if i >= n {
			break
		}
		freqmap := make(map[string]int)
		for j := 0; j < m; j++ {
			freqmap[string(carbons[j][i])]++
		}
		newCarbons := []string{}
		if freqmap["1"] == freqmap["0"] { // keep the carbons with 0
			for k := 0; k < m; k++ {
				if string(carbons[k][i]) == "1" {
					continue
				}
				newCarbons = append(newCarbons, carbons[k])
			}
		}
		if freqmap["1"] > freqmap["0"] {
			for k := 0; k < m; k++ {
...
func calculateRatings(input []string) int {
	binaryOxygen := calculateOxygenRatings(input)
	decimalOxygenRating := convIntoDecimal(binaryOxygen)
	binaryCarbon := calculateCarbonRatings(input)
	decimalCarbonRating := convIntoDecimal(binaryCarbon)
	return decimalOxygenRating * decimalCarbonRating
}
