...
func main() {
	fs := fileScanner()
	leftH, rightH := initHeaps()
	for fs.Scan() {
		leftNum, rightNum := parse(fs.Text())
		heap.Push(leftH, leftNum)
		heap.Push(rightH, rightNum)
	}
	total := calculate(leftH, rightH)
...
func initHeaps() (*minHeap, *minHeap) {
	h1, h2 := &minHeap{}, &minHeap{}
	heap.Init(h1)
	heap.Init(h2)
	return h1, h2
}
...
func calculate(leftH, rightL *minHeap) int {
	n, total := leftH.Len(), 0
	for i := 0; i < n; i++ {
		lSmallest, rSmallest := heap.Pop(leftH).(int), heap.Pop(rightL).(int)
		dist := abs(lSmallest - rSmallest)
		total += dist
	}
	return total
}
